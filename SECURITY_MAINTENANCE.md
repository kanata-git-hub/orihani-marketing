...
Marketing rejects client-supplied SDK transport settings and non-UI models. It keeps Google Search, reference images and existing image sizes. Blog HTML is sanitized for preview and clipboard. Server output is in `server-dist/`, outside public `dist/`. Approved users create a rules-checked reference to their existing auto-ID approval; shared clinic blog history remains shared among approved users. Deleting the original approval revokes its reference. Old Firestore rules may temporarily reject reference creation during rollout without blocking login. Apply the updated rules and refresh logged-in tabs to enable the complete database protection.
