...
code = code.replace(/const imageSectionMatch = text.match\(\/### 1\\\. Image Generation Prompts\.\*\?\(\[\\\\s\\\\S\]\*\?\)\(\?=\\### 2\\\. Video Generation Prompts\)\/i\);/g, `const imageSectionMatch = text.match(/(?:### )?1\\\. Image Generation Prompts.*?(?:[\\s\\S]*?)(?=(?:### )?2\\\. Video|$)/i);`);
